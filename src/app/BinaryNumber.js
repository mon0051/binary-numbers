    ...
    this.add = function (number) {
        ...
        if(n.state.fraction.toJsNumber() === 0 && that.state.fraction.toJsNumber() === 0){
            return new BinaryNumber(
                new MixedNumeral(binaryAdd(this.state.integer, n.state.integer),
                    new BinaryFraction(0,1)));
        }

        that.state = new BinaryNumber(
            new MixedNumeral(binaryAdd(this.state.integer,n.state.integer),
                binaryFractionAdd(this.state.fraction,n.state.fraction)));
